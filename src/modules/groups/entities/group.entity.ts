...
export class Group {
    ...
    @ApiProperty({ type: () => User, description: 'The user who created the group' })
    createdByUser: User;

    @OneToMany(() => GroupMember, groupMember => groupMember.group, { cascade: true })
    @ApiProperty({ type: () => [GroupMember], description: 'The members of the group' })
    members: GroupMember[];

    @OneToMany(() => Message, message => message.group, { cascade: true })
    @ApiProperty({ type: () => [Message], description: 'The messages in the group' })
    messages: Message[];
}
