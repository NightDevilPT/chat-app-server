...
export class GroupMember {
    ...
    @Column()
    @ApiProperty({ description: 'The ID of the user who created the group membership' })
    createdBy: string;

    @ManyToOne(() => Group, group => group.members)
    @ApiProperty({ type: () => Group, description: 'The group this membership belongs to' })
    group: Group;

    @ManyToOne(() => User, user => user.groupMemberships)
    @ApiProperty({ type: () => User, description: 'The user who is a member of the group' })
    user: User;
}
